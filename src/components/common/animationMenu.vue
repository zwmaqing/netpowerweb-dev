<template>
  <transition name="move">
    <div class="menu" v-show="item.menuShow">
      ...
      <div class="inner inner-3" @click="_del(item,index, path)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      _del(item, index, currentPath) {
        this.form.fileName = currentPath + "/" + item.Name;
        this.$confirm('此操作将永久删除该文件(请确认此文件是否已引用), 是否继续?', '提示', {
      ...
      addToSelectTask() {
        ...
        if (this.form.TaskID == 100 || this.form.TaskID == 129) {
          this.$message({
            type: 'warning',
            message: '不能向此任务添加曲目!(应指定曲目位置)'
          });
          return;
        }
