<template>
  <div class="grig">
    ...
    <div class="row">
      ...
      <div class="veluem">
        <el-input v-model="fastButton3.speakTxt" :maxlength=20 placeholder="播报内容20字以内 (未开放)" :disabled="true"></el-input>
      </div>
      <div class="opret">
        <el-button type="primary" icon="el-icon-success" @click="saveFastButton(3)">保存</el-button>
      </div>
    </div>
    <p class="titel">自定义快捷功能按键 2</p>
    <div class="row">
      <div class="name">开关分区分组</div>
      <div class="veluem">
    ...
    <div class="row">
      <div class="name">播报语音内容</div>
      <div class="veluem">
        <el-input v-model="fastButton4.speakTxt" :maxlength=20 placeholder="播报内容20字以内 (未开放)" :disabled="true"></el-input>
      </div>
      <div class="opret">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions(["getGroupsTotal", "getGroups", "getAllGroups2p", "getChannals"]),
      getfastButtonSet() {
        let data = {
          CMD: "GetButton",
          Token: this.tokenStr
  ...
  .grig {
    ...
    .row {
      ...
      .name {
        ...
        display: flex;
        align-items: center;
        margin-left: 6px; //border: 1px solid #aaa;
      }
      .veluem {
        width: 40%; //height: 32px;
        display: flex;
        align-items: center;
        justify-content: center; //border: 1px solid #aaa;
      }
      .opret {
